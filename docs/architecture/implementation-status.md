...
| CHI participant / VirtualDut composition | authority/domain 已先于 feature flow/runtime 闭合，但部分 resolved fixture 和 showcase 仍先建立裸 VirtualDut topology，再把 participant-only fixture 返回的 component 绑定到另一份 canonical VirtualDut；这不是 Home authority 的正确构造入口 | 增加接收既有 canonical VirtualDut、CacheCore 与 port-channel map 的 binder，并迁移 resolved showcase；resolver 继续只闭合/核对，不创建或修改 VirtualDut |
