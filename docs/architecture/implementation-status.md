...
- CHI Issue H 已完成构造依赖、判定作用域与表示/运输三张视图的边界审计，并有受限 direct-Home
  ...
  `FullLineBackingCore` 落地；clean `Evict`、`MakeUnique`、自动 victim/writeback scheduling、一般 `SD`
  生成与 MOESI Owned lifecycle、真实 snoop filter 及独立 HN→SN protocol commit 仍未进入该 slice。两级 XP 的公开
