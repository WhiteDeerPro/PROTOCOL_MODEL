...
- CHI Issue H 已完成构造依赖、判定作用域与表示/运输三张视图的边界审计，并有受限 direct-Home
  ...
  `ChiCoherenceNetworkSession` 分别执行两个终态。modifier 当前不组合 same-line Snoop、Retry 或 error。
  Home attach/canonical binder 已用独立
  `FullLineBackingCore` 落地，并可保持调用方 clean-residency core 的对象身份。该 slice 的 CHI
  lifecycle/profile 尚未加入 deliberate dirty invalidate、WriteEvictFull CAH 的 same-line/post-DBID-Snoop、
