"""AXI4-Lite-bound address fabric VirtualDut recipes."""
...
def build_axi4_lite_address_fabric_vdut(
    name: str,
    protocol: LinkProtocol,
    ...
    *,
    ...
    """Construct a synchronous single-ingress Lite decoder/response mux."""
    ...
    for route in routes:
        ...
        if output_base + route.size_bytes > address_limit:
            raise ValueError(
                f"route {route.name!r} output window exceeds AXI4-Lite address width"
            )
    ...
    if unknown_capabilities:
        raise ValueError(
            "capabilities reference unknown fabric ports: "
    ...
    builder = (
        ...
        .describe(
            "single-ingress AXI4-Lite address fabric (decoder + response mux)"
        )
    )
