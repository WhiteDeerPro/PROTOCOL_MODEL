...
Project 源码目录和 `docs/` 不得保存运行报告、图片快照或图形中间文件。所有运行证据只能
进入带 manifest 的 `out/<project>/<run-id>/`；需要复查时重新运行，不人工复制产物。
...
├── cases/
│   └── <case>/
│       ├── trace.json       本 case 的规范事件、采样与 fault
│       ├── waveform*.svg    本 case 的采样层证据
│       └── causality*.svg   本 case 的事件依赖证据
└── sources/cases/      对应 case 的 WaveJSON 和 Graphviz DOT 源文件
```
...
一个 run 表示一次完整实验，legal witness 与一个或多个 negative mutation 是其中的多个 case，
不是互不相关的多次实验。共享拓扑、协议实例和约束只在 run 根目录保存一次；每个 case 独立保存
trace、波形与因果图。manifest v2 会在每个 case 下列出所属 artifacts，总报告则把这些结果放在
同一上下文中比较。
...
6. 确认所有运行产物均被 manifest 登记，且没有复制进源码或文档目录。

运行全部维护中的 Project 可使用：

```bash
.venv/bin/python -m protocol_model run-all
```

命令同时生成 `out/index.html`，作为所有 Project 报告的浏览入口。首次使用可直接运行
`./scripts/quickstart.sh`，脚本只负责准备本地 venv/WaveDrom、检查 Graphviz 并调用同一 CLI，
不会另建一套实验语义。
