...
- 准备发布时，按 [宣称与证据审计](../strategy/claims-and-evidence.md) 复核所有 `CURRENT`、`PROPOSED` 和
  `RESEARCH QUESTION` 标签；受众、渠道、阶段和内容取舍见 [传播策略](../strategy/README.md)。

当前稿件采用“24 个 AXI4 场景，其中 2 个在同一导航内精讲”的时间快照。数字和状态变化时先更新审计底稿，
再统一修改本目录的公开稿；不要在本 README 制定另一套推广计划。
