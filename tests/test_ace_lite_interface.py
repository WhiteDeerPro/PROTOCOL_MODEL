...
class AceLiteDataInterfaceTest(unittest.TestCase):
    def test_native_address_schema_and_roles(self) -> None:
        protocol = build_ace_lite_data_interface()

        self.assertEqual("amba.ace_lite", protocol.interface_family)
        self.assertEqual(
            {"manager", "coherent_interconnect"}, set(protocol.roles)
        )
        self.assertTrue(
            {"domain", "snoop", "bar"}
            <= set(protocol.event_kinds["AR"].schema.fields)
        )
        self.assertEqual(
            4, protocol.event_kinds["AR"].schema.fields["snoop"].domain.width
        )
        self.assertEqual(
            3, protocol.event_kinds["AW"].schema.fields["snoop"].domain.width
        )
    ...
    def test_cacheable_transaction_rejects_system_domain(self) -> None:
        ...
        session = build_ace_lite_data_interface().open_session()
