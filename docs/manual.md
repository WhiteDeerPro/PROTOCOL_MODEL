...
Protocol Model 为这些问题提供一个可执行模型。它不是 RTL 仿真器、综合工具或 UVM
替代品；它关注协议语义。它把时钟边沿的引脚观测降低为规范事件，再检查事件之间的状态、
数据、数量和因果关系。
...
```text
protocol_model/projects/prj_ready_valid_sink/sims/01/index.html
protocol_model/projects/prj_axi4_read_bridge/sims/01/index.html
```

`sims/` 是运行产物，未被 Git 跟踪，可删除后重新生成。需要保存到别处时：

```bash
.venv/bin/python -m protocol_model ready-valid-sink --sim-dir /tmp/ready-valid-run
.venv/bin/python -m protocol_model axi-read-network --sim-dir /tmp/axi-read-run
```
...
`tests/` 保留为语义核心的回归资产。它们不替代上述 witness 和可视化证据：测试保护实现不
倒退，证据说明模型为何接受或拒绝一段行为。
