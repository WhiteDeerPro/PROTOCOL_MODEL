...
- CHI Issue H 已完成构造依赖、判定作用域与表示/运输三张视图的边界审计，并有受限 direct-Home
  ...
  CleanUnique 被同址失效后，`Comp_UC` 形成不持有 payload 的 `UCE`。dirty-unique 扩展再加入
  ...
  形成 `UCE`，full-line local write 再完成 `UCE→UD`。direct 双 Requester witness 已闭合同址 CleanUnique
  ...
  `FullLineBackingCore` 落地；`MakeUnique`、自动 victim/writeback scheduling、Evict Retry/deliberate
