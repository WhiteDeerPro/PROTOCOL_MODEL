"""AHB-bound address fabric VirtualDut recipes."""
...
def build_ahb_address_fabric_vdut(
    ...
    *,
    ...
    """Construct a synchronous single-ingress AHB decoder/response mux."""

    address_limit = 1 << int(protocol.parameters["address_width"])
    for route in routes:
        if route.limit_address > address_limit:
            raise ValueError(
                f"route {route.name!r} input window exceeds AHB address width"
            )
        output_base = (
            route.base_address
    ...
    egress = {
        port: _binding(
            port,
            protocol,
            AhbRequesterAttachment(protocol, byte_order=byte_order),
