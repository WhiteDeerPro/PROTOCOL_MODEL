...
DOT、WaveJSON、SVG 和 HTML 是表示或渲染格式，不是视图语义。相同的 DOT 可以表达 topology、因果关系或
消息序列；把它们都称为“Graphviz 图”会丢失阅读边界。当前工程中的图和报告可以归入以下六类。
...
最后一类是 artifact，但通常不是“图”。宣传用 overview、architecture explanation 和 decision view 可复用结构图或
分析图的投影模型，同时标记为解释性材料；无需因为它是手工编排的宣传图再创造一套协议语义。
...
- **信号波形与事件波形**都可以由 WaveDrom 绘制，但横轴含义不同。信号波形通常以 clock tick/cycle 为基准；
  事件波形可能只是 timestamp、model step 或 event index。后者必须显式写出时间基准，不能让读者误认为 RTL
  周期波形。
...
不建议继续依赖一个自由字符串 `kind` 同时表达视图类型、作用域和文件角色。较稳定的描述至少包含：

| 字段 | 示例 | 作用 |
|---|---|---|
| `view_kind` | `topology`、`signal_timing`、`message_sequence`、`causal_graph` | 图在语义上表达什么 |
...
| `time_basis` | `none`、`clock_tick`、`timestamp`、`event_index`、`causal_rank` | 横轴或排序的真实含义 |
| `source_schema` | `protocol-model.topology/v1` | 投影数据的可演进格式 |
| `renderer` | `graphviz.dot`、`wavedrom.wavejson` | 怎样把 source artifact 变成可阅读文件 |
| `detail` | `overview`、`standard`、`diagnostic` | 选择显示密度；不改变 ViewIR 中保存的事实 |
...
4. artifact store 保存 source、rendered 文件及二者的派生关系；
5. report/gallery 从 manifest 选取材料，不重新定义模型结论。
...
showcase / publishing composition root
                 ...
                 ▼
             artifacts
       record + store + manifest + provenance

visualization 和 artifacts 不直接拥有彼此的生命周期
```

- `protocol_model.visualization` 拥有视图模型和投影规则，不决定一次运行写到哪里，也不维护发布目录。
- `protocol_model.artifacts` 对协议族无感，拥有文件记录、写入生命周期和 provenance；它不理解 AXI channel、
  CHI message 或 VirtualDut backend。
...
引用。中央 rectangle 只是一个多端口 VirtualDut 的边界投影，不表示物理共享总线、crosspoint 数量或内部并行通道。

目前还有两处过渡状态：多数 system view 仍直接生成 DOT；VirtualDut structure 与 address interconnect 已有 typed
view，但普通 topology、trace 和 bus strip 尚未迁移；`VisualizationPublisher` 同时接触 renderer 与
`RunArtifactStore`。后续可以先统一 view descriptor 和 kind 命名，再决定是否把 publisher 移到独立 composition
包。无需为此重写现有图或建立万能布局引擎。
...
- 当 trace 大到静态 SVG 难以浏览时，可参考 Perfetto 一类 track/slice/flow 模型提供缩放、筛选和跨轨关联；
  这是以后扩展 renderer 的方向，不需要进入协议核心。
- provenance 可以借鉴 W3C PROV 的 entity/activity/agent 与 derivation 思路，但当前 manifest 只需要记录工程可
  重建所需的生成入口、参数、source/render 关系和版本，不必实现完整标准。
...
1. 建立有限的 `view_kind` 词表，并把 `topology`/`system_topology`/`topology_projection` 等历史字符串映射到它；
2. 在 artifact record 中增加上述正交 descriptor，同时保留 source/render 派生关系；
3. 先为 system topology、signal timing、message sequence、causal graph 和 VirtualDut structure 建立最小 ViewIR；
4. 让报告和 gallery 只读 manifest 自动生成导航；
5. 只有真实的大规模 trace 场景出现后，再评估交互式 viewer。

这是一条渐进迁移路线。现有 DOT/WaveJSON 与 SVG 不需要立即重做，旧 `kind` 也可以在 descriptor 稳定前作为
输入别名；需要优先消除的是含义混用，而不是文件格式差异。
