...
本文是 bridge/Transform 的 canonical 架构说明。当前代码接入顺序和验收边界单独记录在
[事务转译 V1 实施计划](translation-implementation.md)。
...
| 下一步接入 | attachment-aware 事务式 encode/decode 外壳、`AddressBurst`/address stages，以及 AXI4→APB 对公共 executor 的接入 |
...
源码职责、实施阶段和暂缓能力见 [V1 实施计划](translation-implementation.md)。
