...
class ChiIssueHCleanUniqueSystemTest(unittest.TestCase):
    ...
    def build_resolved(
        ...
        *,
        ...
        builder = SystemProtocolBuilder(
            (
                "chi_clean_unique_clean_peer_via_xp"
                if via_xp
        ...
        if via_xp:
            ...
            connection_specs = self._xp_connections(include_peer_rsp)
            home_claim_port = "rx_from_xp"
        else:
            self._add_direct_duts(builder)
            connection_specs = self._direct_connections(include_peer_rsp)
        ...
        if via_xp:
            ...
            peer_channels = {
                "tx_to_xp": frozenset((ChiChannelKind.RSP,)),
            ...
            initial_directory=(
                ChiHomeDirectoryEntry(
                    ...
                    sharers=(
                        ...
                        if peer_state is ChiCacheState.SC
                        else frozenset()
                    ),
                    unique_owner=(
                        self.PEER
        ...
        home_binding = self._home_binding(duts, home, via_xp=via_xp)
        facets = [
    ...
    def _direct_connections(self, include_peer_rsp: bool):
        items = [
            (
                "request_ack",
    ...
    def _xp_connections(self, include_peer_rsp: bool):
        items = [
            (
                "rn0_to_xp",
    ...
    def _router_binding(self, duts) -> ChiParticipantBinding:
        ...
        channels = {
            ...
            "from_rn1": frozenset((ChiChannelKind.RSP,)),
            "from_hn0": frozenset(
                (ChiChannelKind.RSP, ChiChannelKind.SNP)
            ),
    ...
    def test_dirty_peer_is_rejected_before_network_run(self) -> None:
