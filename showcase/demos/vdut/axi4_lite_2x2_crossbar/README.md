...
`DutAdvanceAction` 表示一次显式 service opportunity；硬件周期由另外的 timing/driver profile 定义。
...
service opportunity，状态 lane 是该步结束后的 crossbar post-state。ACLK、VALID/READY 和 RTL pin
由 pin/cycle observation 视图表达。
...
比较 operation/effect、correlation 和必要偏序。具名 `PIN_CYCLE` contract 可以把相应周期安排纳入 golden
