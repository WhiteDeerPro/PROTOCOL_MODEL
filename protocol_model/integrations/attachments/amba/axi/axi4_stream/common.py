...
from protocol_model.protocols.amba.axi.axi4_stream import AXI4_STREAM_FAMILY


def require_axi4_stream_role(protocol: InterfaceProtocol, role: str) -> None:
    if protocol.interface_family != AXI4_STREAM_FAMILY:
        raise ValueError(
            "AXI4-Stream attachment requires protocol family "
            f"{AXI4_STREAM_FAMILY!r}, got {protocol.interface_family!r}"
        )
    if role not in protocol.roles:
        raise ValueError(
            f"AXI4-Stream protocol has no role {role!r}"
        )
    if set(protocol.event_kinds) != {"T"}:
