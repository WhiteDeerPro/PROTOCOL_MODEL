...
- CHI Issue H 已完成构造依赖、判定作用域与表示/运输三张视图的边界审计，并有受限 direct-Home
  ...
  backing 或 clean residency。该窄组合不包含 `UC→SC`/非失效 Snoop；Home 已发 `Comp` 后须等
  `CompAck`、已发 `CompDBIDResp` 后须等 DAT，post-response 同址 Snoop作为 ordering 负例而非正向
  ...
  lifecycle/profile 尚未加入 deliberate dirty invalidate、WriteEvictFull CAH 的 `UC→SC`/非失效 Snoop、
