...
from .definition import build_axi4_interface
...
class Axi4ObservationPolicy:
    """Pin-observation restrictions kept separate from InterfaceProtocol profiles."""
...
class Axi4ObservationSession(
    ...
    """Lower five ready/valid lanes into one atomic InterfaceSession batch."""
    ...
    def __init__(
        ...
        *,
        ...
        if set(self.protocol.event_kinds) != expected:
            raise ValueError("AXI4 observation requires the five-channel interface")
        ...
        for name, channel in self.protocol.event_kinds.items():
    ...
    def initial_state(self) -> Axi4ObservationState:
        return Axi4ObservationState(
            ...
            self.interface_session.initial_state(),
        )
    ...
    def is_quiescent(self, state: Axi4ObservationState) -> bool:
        return self.interface_session.is_quiescent(state.interface_state) and all(
    ...
    def step(
        ...
        link_transition = self.interface_session.step_batch(state.interface_state, events)
