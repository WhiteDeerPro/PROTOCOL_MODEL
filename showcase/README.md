...
| CHI Issue H flow gallery | 比较 5 个 coherence/progress 流程的实际拓扑/交互边界、transaction 时空、显式因果关系与语义事件时间线 | [生成证据](generated/chi/issue-h-flow-gallery/README.md) | [`demos/chi/issue_h_flow_gallery/`](demos/chi/issue_h_flow_gallery/README.md) |
...
图是模型执行的投影，不是 raw RTL/VCD 采样。CHI flow gallery 的 topology 来自各案 resolved
construction；仅 participant-level 执行的案例会明确降级为交互边界。timeline 以语义事件为时间基准，
