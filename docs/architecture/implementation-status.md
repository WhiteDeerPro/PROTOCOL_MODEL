...
- CHI Issue H 已完成构造依赖、判定作用域与表示/运输三张视图的边界审计，并有受限 direct-Home
  ...
  派生。pending `ReadUnique` 与同址 `SnpUnique` 的 RN transient 已允许 `I/SC→I` Snoop 响应而不丢失
  pending/Retry correlation，并由随后 `CompData_UC` 重新安装 line；CleanUnique/WriteBack 同址 Snoop、
  等待者合并/优先级和一般多 Requester resolved topology 仍未闭合。Home attach/canonical binder 已用独立
