...
| CHI read/coherence lifecycle | 受限 direct-Home `ReadNoSnp→CompData`、clean `ReadShared`/`ReadUnique`、dirty-owner transfer、MESI no-SD `ReadNotSharedDirty`、显式 `UD` `WriteBackFull` 与 clean-peer `CleanUnique` 已在 participant、packet-delivery session 和 topology-backed network session 中闭合。`CleanUnique` 当前执行 `SC requester → CleanUnique → SnpCleanInvalid → clean SnpResp_I → Comp_UC → CompAck → UC requester`：不重取 requester 已有数据，Home backing 不变，directory 在匹配 DBID 的 CompAck 后提交；direct 与单 XP witness 均检查五个 packet、五类 REQ/SNP/RSP flow 和无 DAT evidence。dirty requester/peer、`SnpRespData_I_PD`、Home memory update 与 downstream backing commit 明确不属于这一 clean-only feature。协议中立 `CacheLineStore/CacheCore` 持有 resident payload，RN participant 持有 `I/UC/SC/UD` permission 与共享 TxnID/capacity 的 `pending_transactions`，Home 持有 directory、backing、Snoop TxnID 与 DBID pending；静止点 monitor 检查 directory、permission 和相应的数据权威。Cache behavior 可绑定到已有 canonical VirtualDut，binder 不创建 port 或 topology connection。六个 coherence profile 分别登记为构造期 feature；CleanUnique 合同闭合 Requester→Home REQ、Home→Snoopee SNP、Snoopee→Home RSP、Home→Requester RSP 与 Requester→Home CompAck RSP，不声明 DAT。Home/Snoopee 仍从 selected address claim、Home authority 与 coherence domain 派生，network session 从 closed flow evidence 建立 route 并自动推进 participant egress、Link/router 与 endpoint delivery；场景只提交 operation 并显式调用离散 advance/run。当前缺口包括 dirty-peer CleanUnique、`MakeUnique`、clean `Evict`、自动 victim/writeback scheduling、Retry/error 与 coherence 的组合、完整 same-line transient/hazard、多 pending emission batch、`SD`/Owned、forwarding snoop、真实 snoop filter、narrow/error completion、多 waiter公平性和 multi-Home/SAM runtime；有界运行耗尽仍为 inconclusive，不是 deadlock proof |
...
| CHI participant / VirtualDut composition | cache binder 已接收既有 canonical VirtualDut、CacheCore 与 port-channel map；resolved fixture 和 clean 2×2 mesh showcase 不再把另一份同名 VirtualDut 产生的 component 重绑到 topology object。binder 只形成 participant/facet，不连接网络；resolver 核对 object identity、NodeID、channel flow 与 authority，不创建或修改 VirtualDut | 第二种需要共享此构造协议的 participant 类型出现时，再提取通用 participant/identity binder API；拓扑连接仍由 SystemProtocol construction 显式建立 |
...
- CHI Issue H 已完成构造依赖、判定作用域与表示/运输三张视图的边界审计，并有受限 direct-Home
  ...
  两个 clean shared copy。这些 lifecycle 分别登记为构造期 feature；dirty-unique 依赖 clean ReadUnique，而独立
  ...
  clean-peer `SC→CleanUnique→UC` 已进入 participant lifecycle；集合从 coherence domain 自动派生。
  dirty-peer `CleanUnique`、`MakeUnique`、clean `Evict`、自动 victim/writeback scheduling、
