...
**审计快照 / Review snapshot:** 2026-07-22
...
|---|---|---|---|---|
| C-01 | Protocol Model 可以从 typed event、constraint、resource、obligation 和可组合 fragment 构造协议语义。 | Protocol Model composes protocol semantics from typed events, constraints, resources, obligations, and fragments. | [`semantics`](../../protocol_model/semantics/)；[`patterns`](../../protocol_model/patterns/)；[当前状态](../architecture/implementation-status.md) | “组合”指当前 Python 模型的构造与执行，不是数学完备性证明。 |
| C-02 | 当前 AXI4 InterfaceProtocol 覆盖 burst、narrow/unaligned、read interleave、AW/W/B correlation、interface-local exclusive eligibility 和 observation。 | The current AXI4 InterfaceProtocol covers bursts, narrow/unaligned transfers, read interleaving, AW/W/B correlation, interface-local exclusive eligibility, and observation. | [`protocols/amba/axi/axi4`](../../protocol_model/protocols/amba/axi/axi4/)；[`test_axi4_interface.py`](../../tests/test_axi4_interface.py)；[`test_axi4_narrow.py`](../../tests/test_axi4_narrow.py)；[`test_axi4_exclusive.py`](../../tests/test_axi4_exclusive.py)；[`test_axi4_observation.py`](../../tests/test_axi4_observation.py) | 不写“完整 AXI4 compliance”；跨接口 exclusive conflict、memory visibility 和逐条 requirement catalog 尚未闭合。 |
| C-03 | 当前主线包含 AXI4、AXI4-Lite、AXI4-Stream、AHB-Lite/AHB5 profile、APB3/4/5 和 ACE-Lite ordinary-data subset 的 InterfaceProtocol 实现。 | The main line includes InterfaceProtocol implementations for AXI4, AXI4-Lite, AXI4-Stream, AHB-Lite/AHB5 profiles, APB3/4/5, and an ACE-Lite ordinary-data subset. | [`protocols/amba`](../../protocol_model/protocols/amba/)；[`test_ahb_interface.py`](../../tests/test_ahb_interface.py)；[`test_apb_interface.py`](../../tests/test_apb_interface.py)；[`test_ace_lite_interface.py`](../../tests/test_ace_lite_interface.py) | 不写“全部 AMBA”。CHI Issue H 当前另有受限 representation/transport/participant/system slice；TileLink 仍只有 namespace。 |
...
| C-05 | SystemProtocol 可以连接多个 VirtualDut 与 interface/transport connection，检查 ownership/elaboration，并在当前同步边界内运行微小网络。 | SystemProtocol can connect VirtualDuts through interface and transport connections, check ownership and elaboration, and execute small networks within the current synchronous boundary. | [`system`](../../protocol_model/system/)；[`test_system_protocol.py`](../../tests/test_system_protocol.py)；[SystemProtocol 架构](../architecture/system-protocol.md) | 当前 interface runtime 使用同步 fixed-point；异步 emission、复杂 progress 和 deadlock 尚未闭合。 |
| C-06 | 统一 AMBA serial bridge builder 可装配 AXI4、AXI4-Lite、AHB、APB 的单入口/单出口组合，并复用 typed translation backend。 | The shared AMBA serial bridge builder assembles single-ingress/single-egress AXI4, AXI4-Lite, AHB, and APB combinations over a typed translation backend. | [`integrations/recipes/amba/bridges`](../../protocol_model/integrations/recipes/amba/bridges/)；[`test_virtual_dut_amba_serial_bridges.py`](../../tests/test_virtual_dut_amba_serial_bridges.py)；[`test_amba_bridge_chain.py`](../../tests/test_amba_bridge_chain.py) | 当前 profile 严格串行，默认同宽；width split/merge、burst-preserving egress、有限 ID remap pool 和并发 child 尚未实现。 |
| C-07 | 类型化事务转译内核已经实现 operation signature、stage contract、plan closure、fan-out lifecycle、capacity lease 和 serial executor。 | The typed transaction translation kernel implements operation signatures, stage contracts, plan closure, fan-out lifecycle, capacity leases, and a serial executor. | [`virtual_dut/translation`](../../protocol_model/virtual_dut/translation/)；[`test_virtual_dut_translation.py`](../../tests/test_virtual_dut_translation.py)；[转译架构](../architecture/typed-transaction-translation.md) | 这里的“typed”是 Python 类型对象与构造期/运行期合同检查，不是静态类型定理或自动 bridge 合成证明。 |
...
| C-09 | 当前统一 AXI4 介绍集实际执行 24 个具名场景；每案生成模型波形、因果图和机器结果，其中两个场景在同一导航内增加逐步精讲。 | The unified AXI4 introduction set executes 24 named scenarios; every case produces a model waveform, causal graph, and machine-readable result, while two cases add focused walkthroughs within the same navigation. | [`source and runner`](../../showcase/demos/axi4/README.md)；[`中文发布结果`](../../showcase/generated/axi4/README.zh-CN.md)；[`English artifacts`](../../showcase/generated/axi4/README.en.md)；[`coverage graphic`](../../showcase/generated/axi4/coverage.svg) | 10 个合法场景与 14 个预期违规满足声明期望；这是代表性场景证据，不是 AXI4 规范条款覆盖率或 RTL compliance。18 个 event 输入场景生成 CanonicalEvent 顺序视图，6 个 frame 输入场景生成 `AtomicFrame` ready/valid 视图；其中两案增加精讲。两种视图都不是 RTL/VCD。 |
| C-10 | 当前 AXI4-Lite SystemProtocol 示例可以执行一个 manager 经显式 address fabric 访问三个 subordinate，并从同一 topology 生成星形图和传统 bus-strip 图。 | The current AXI4-Lite SystemProtocol example executes one manager accessing three subordinates through an explicit address fabric and derives both star and bus-strip views from the same topology. | [`source`](../../showcase/demos/system/axi4_lite_single_manager_fabric/README.md)；[`generated witness`](../../showcase/generated/system/axi4-lite-single-manager-fabric/README.md)；[`system_bus_strip_dot`](../../protocol_model/visualization/system.py) | 当前是 AXI4-Lite 单入口 fabric；bus-strip 只是折叠显示，不表示隐式 multi-drop AXI connection，也不覆盖 full AXI4 burst/ID routing。 |
| C-11 | 当前受限 CHI Issue H 示例可以在调用方声明的两级 XP topology 上执行三跳 REQ、反向三跳 DAT，并保留端到端 completion lineage。 | A restricted CHI Issue H example executes a three-hop REQ and reverse three-hop DAT path over a caller-declared two-XP topology while retaining end-to-end completion lineage. | [`source`](../../showcase/demos/system/chi_issue_h_routed_read/README.md)；[`generated witness`](../../showcase/generated/system/chi-issue-h-routed-read/README.md)；[`CHI system runtime`](../../protocol_model/protocols/amba/chi/issue_h/system/) | 这是 direct-Home、single-DAT-flit、REQ/DAT-only witness；不包含完整 CHI Port、通用外设 attachment、SNP/coherence、QoS/fairness 或 deadlock proof。 |
...
| P-03 | “公共 translation kernel 将继续扩展 bridge 的并发、宽度与 burst-preserving profile。” / “The shared translation kernel will continue to add concurrent, width-converting, and burst-preserving bridge profiles.” | 当前多协议 preset 已复用 serial executor，但 child 发行、ID remap 与宽度转换仍受限。 | 至少一个并发或 width-converting preset 复用现有 stage/executor，并显式验证 ordering、capacity 与 error folding。 |
...
| P-05 | “后续小型 interconnect 示例将把现有 arbitration、return ownership 与 typed blocked reason 推进为 wait-for/deadlock 分析。” / “A later interconnect example will extend the existing arbitration, return ownership, and typed blocked reasons into wait-for/deadlock analysis.” | 2×2 address crossbar 已有 round-robin、owner table 和有限入口队列；held-resource edge、response-path capacity、wait-for graph 与 progress property 尚未闭合。 | 从实际 crossbar/network state 派生 resource ownership 与 wait edge，在有界场景中区分临时 backpressure、潜在环和可证明 deadlock；不能只把一次 scheduler budget 耗尽写成死锁。 |
...
| RQ-03 | InterfaceProtocol / VirtualDut / SystemProtocol 的作用域是否改善故障归属？ | Does the InterfaceProtocol / VirtualDut / SystemProtocol scope split improve fault ownership? | 用 interface-local、bridge-local 和 system-level 三类真实缺陷做盲审，看诊断是否落到正确 owner 与证据窗口。 |
...
| “在工业界首次实现……” | 无法从当前仓库证明 first-to-market 或穷尽专有工具能力。 | “把 InterfaceProtocol、VirtualDut、SystemProtocol 与 typed translation 放在一个参考实现中联合探索。” |
