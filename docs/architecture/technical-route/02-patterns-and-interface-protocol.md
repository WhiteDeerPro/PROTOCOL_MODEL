...
描述符和数据需要汇合、同一 ID 内不能越序。Pattern 抽取这些形状，InterfaceProtocol 再选择并组合它们。
...
它是不可变定义，不保存某次运行的 pending request。每条具体 InterfaceConnection 都需要自己的
InterfaceSession，这样两个 AXI interface connections 不会意外共享 outstanding 状态。
...
字段、方向和关键参数。它不是完整的标准族谱记录；当 revision 与 compatibility policy 需要独立演化时，可再
拆出 `standard_family`、`revision` 和 compatibility key。两个独立构造但配置相同的 AXI InterfaceProtocol
可以共享 attachment；语义 monitor 的函数对象身份不参与 interface shape 比较。
...
## 5. InterfaceProtocol 的作用域边界

InterfaceProtocol 可以判断：

- 字段宽度、枚举和事件局部几何；
- event direction；
- request/response 数量与关联；
- burst、同 ID ordering、单 interface outstanding resource。
...
- 多条连接的 route、owner、deadlock——SystemProtocol。

Interface-local 是判定作用域，不是 CHI 的 Link layer。CHI flit、L-Credit 和 link activation 可以复用本页
使用的 EventSchema、monitor、resource 与 session product，但由 transport contract 组织；CHI transaction、
packet、flit 的边界见[通信建模的三张视图](../communication-scope-and-transport.md)。
...
详细协议说明仍在 [AXI4](../axi4-interface.md)、[AMBA variants](../amba-interface-variants.md) 和
