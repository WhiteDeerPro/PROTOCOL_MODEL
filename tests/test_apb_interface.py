...
from protocol_model.protocols.amba.apb.apb3 import (
    Apb3Config,
    Apb3ObservationSession,
    Apb3Signals,
    build_apb3_interface,
)
from protocol_model.protocols.amba.apb.apb4 import (
    Apb4Config,
    Apb4ObservationSession,
    Apb4Signals,
    build_apb4_interface,
)
from protocol_model.protocols.amba.apb.apb5 import (
    Apb5Config,
...
class ApbInterfaceTest(unittest.TestCase):
    def test_apb_revisions_have_independent_public_schemas(self) -> None:
        apb3 = build_apb3_interface()
        apb4 = build_apb4_interface()
        apb5 = build_apb5_interface()

        self.assertEqual(APB_FAMILY, apb3.interface_family)
        self.assertEqual(APB_FAMILY, apb4.interface_family)
        self.assertEqual(APB_FAMILY, apb5.interface_family)
        self.assertEqual({"addr"}, set(apb3.event_kinds["READ"].schema.fields))
        self.assertEqual(
            {"addr", "prot"}, set(apb4.event_kinds["READ"].schema.fields)
        )
        self.assertEqual(
            {"addr", "prot", "data", "strb"},
            set(apb4.event_kinds["WRITE"].schema.fields),
        )
        with self.assertRaises(TypeError):
            Apb3Signals(False, False, pprot=0)  # type: ignore[call-arg]

    def test_apb4_resolves_pprot_and_pstrb_independently(self) -> None:
        prot_only = build_apb4_interface(
            Apb4Config(pprot_present=True, pstrb_present=False)
        )
        strb_only = build_apb4_interface(
            Apb4Config(pprot_present=False, pstrb_present=True)
        )

        self.assertEqual(
            {"addr", "prot", "data"},
            set(prot_only.event_kinds["WRITE"].schema.fields),
        )
        self.assertEqual(
            {"addr", "data", "strb"},
            set(strb_only.event_kinds["WRITE"].schema.fields),
        )
        self.assertEqual({"addr"}, set(strb_only.event_kinds["READ"].schema.fields))

    def test_apb_dimensions_follow_protocol_limits(self) -> None:
        for data_width in (8, 16, 32):
            self.assertEqual(
                data_width,
                build_apb3_interface(data_width=data_width).parameters["data_width"],
            )
    ...
    def test_apb5_schema_carries_user_and_rme_attributes(self) -> None:
        ...
        protocol = build_apb5_interface(config)

        self.assertEqual(
            {"addr", "prot", "nse", "auser"},
            set(protocol.event_kinds["READ"].schema.fields),
        )
        self.assertEqual(
            {"addr", "prot", "nse", "auser", "data", "strb", "wuser"},
            set(protocol.event_kinds["WRITE"].schema.fields),
        )
        self.assertEqual(
            {"data", "error", "ruser", "buser"},
            set(protocol.event_kinds["READ_RESPONSE"].schema.fields),
        )
        self.assertEqual(
            {"error", "buser"},
            set(protocol.event_kinds["WRITE_RESPONSE"].schema.fields),
        )
