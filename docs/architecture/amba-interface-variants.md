# AMBA InterfaceProtocol variants

AXI4-Lite 与 AXI4-Stream 都是 `InterfaceProtocol`，但它们和 AXI4 的关系不同：
...
代码位于 `protocol_model/protocols/amba/axi/axi4_lite/`。原生 schema 只包含规范允许的五通道字段：
...
代码位于 `protocol_model/protocols/amba/axi/axi4_stream/`。基础协议只有一个 transmitter → receiver 的 `T` channel。
