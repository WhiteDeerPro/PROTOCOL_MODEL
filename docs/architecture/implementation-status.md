...
| CHI network/router slice | `SystemProtocolBuilder.connect_transport()` 已将有向 CHI connection 放入 canonical topology，elaboration 形成 `ResolvedTransportPlan`。一条 connection 可同时启用 REQ/RSP/SNP/DAT，并以一个 activation state 配合各 channel 独立的 FIFO、receiver reservation 与 L-Credit；lineage 按 `connection + channel` 保存。`ChiTransportNetworkSession` 读取这份 plan 及调用方 router registry，原子提交 capture→router 和 router service→downstream enqueue。有限 store-and-forward router 按 network packet 的 `channel + target_id` 精确路由且不改写 protocol message；Session 不内置 RN→router→Home、ring 或 mesh。SNP message 不拥有协议 `TgtID`；coherent Home 可按 clean Shared/Unique、CleanUnique/MakeUnique invalidate、dirty unique 和 no-SD ReadNotSharedDirty profile 从目录选择目标，并为每个 Snoopee 建立显式 packet copy。通用 multicast switch 与 topology-wide fanout scheduler 尚未实现。`ChiBehaviorFacet` 已区分 transaction/forwarding behavior，NodeID identity plan、flow projector 与 feature capability resolver 可形成 CHI-family 构造期闭合证据。`ChiCoherenceAuthorityContract` 引用通用 `AddressClaim`，将 claim 绑定到 scalar Home 与通常可选的 coherence domain；resolver 从 domain 派生 finite Snoopee set，并按每个成员闭合 SNP 去程、RSP/DAT 回程和 participant capability。clean Shared/Unique、CleanUnique、dirty WriteBack、WriteEvictFull base 与 CopyAtHome modifier 的 requester role 可显式绑定有限集合；Snoopee authority 是各 requester eligible-peer 集合的并集，允许某 RN 对一笔事务是 requester、对另一笔是 Snoopee。当前 resolved witness 使用同构 requester set，不能由这些具名 cardinality 扩展推断异构 per-feature scope 或一般 multi-Requester construction。`WriteEvictFull(CAH=0)`、其 CopyAtHome modifier 与 `WriteEvictOrEvict(CAH=0)` 都不产生 SNP，但因 clean-residency 的 Snoop-domain 边界以 feature authority requirement 强制选择 domain。CopyAtHome modifier 依赖 clean ReadUnique acquisition 和 WEF base，并增加 no-data 分支所需的 Home→Requester `Comp` 与 Requester→Home `CompAck` RSP flow；WEOE capability closure 同时要求 REQ、Home→Requester RSP、Requester→Home DAT 与 CompAck RSP 四条可运行 flow。单笔 conditional CopyBack 再按 Home outcome 使用 DAT 或 CompAck 终态。手工 Home/Snoopee role 不再是 resolved system 的并行权威源。当前每个 feature scope 仍只选择一个 claim/scalar Home，authority window 按 claim 的 system-visible 地址解释；multi-Home runtime、SAM remap/translated authority 与通用 participant/identity API 尚未实现 |
| CHI read/coherence lifecycle | participant、packet-delivery 与 topology-backed session 已闭合 direct `ReadNoSnp`/NDERR/Retry、clean Shared/Unique、dirty-owner transfer、no-SD MESI `ReadNotSharedDirty`、CleanUnique、MakeUnique、clean Evict/Retry、WriteBackFull、`WriteEvictFull(CAH=0)` 及其 response 前 Snoop 组合，以及 `WriteEvictOrEvict(CAH=0)` 的四个正常 outcome。独立 `WriteEvictFull(CAH=1)` modifier 已闭合 clean `ReadUnique→CompData_UC(CAH=1)` provenance acquisition、`CompDBIDResp→CopyBackWrData_UC`/`Comp→CompAck_UC` 两个正常终态，以及 Home response 前的 invalidating Snoop→`I` 与 `SnpShared`→`SC`。后者保留 frozen REQ，清除当前 CAH provenance，并把 pending outcome 记为 `LIVE_SC`；`CAH={0,1}` 的 data 分支都以 `CopyBackWrData_SC` 退休，只有 CAH=1 modifier 可选择 no-data `Comp→CompAck_SC`。`CURRENT_SHARED_HOLDER` 由 system 根据 RN `LIVE_SC`、Home clean-sharer directory 与 backing 一致性派生，不是 REQ wire field 或独立 Home 的猜测。2RN+HN+XP resolved witness 通过 ReadShared 产生 `SnpShared`，再执行 WEF 的 data/no-data 两种 SC terminal；每种场景恰好运输八个 endpoint packet。当前模型仍用显式 `write_evict_full_current_copy_policy` 和 `CHECK_CURRENT_COPY` profile 收窄 no-data 选择：只有 Home 的 Snoop-domain `clean_residency` 仍保存该行时才允许 `Comp`；Home 仍可选择请求数据。该 policy 是首个可执行子集，不把 `CAH=1` 解释成 Home 当前仍有 copy 的协议证明。WEOE response 前 invalidating-Snoop cancel 另由 direct 双 Requester session 证明。直接的 lifecycle/profile 扩展包括 DERR/post-Snoop error、MakeUnique Retry/error/MTE/partial-write、CopyBack 的其余 Retry/error phase、一般 same-line transient、Retry cancel 和可选 DCT forwarding；Home 已发 `Comp`/`CompDBIDResp` 后须先等 `CompAck`/DAT，post-response 同址 Snoop 在当前 profile 中是 ordering 负向边界而非正向 transient。victim/LRU 与容量驱动 outcome 仍属于可选 Home/Cache VirtualDut policy；Owned/SD、snoop filter、HN→SN、multi-Home/SAM、multi-packet DAT 和 deadlock/fairness 分别按 coherence state、system construction、representation/session 与 verification 维度管理，不作为同一项 lifecycle 缺口 |
...
和只读投影，不再是第二份状态权威。RN 成功接收 `CompDBIDResp` 后，再为其实际产生的
...
| CHI CopyBack legacy evidence projection | `ChiCopyBackPhaseLedger` 已成为 WriteBackFull、WriteEvictFull 与 WriteEvictOrEvict 的唯一 exact-evidence 权威，统一 TxnID→DBID phase split、Home DBID namespace 和 DAT/ack terminal；原 operation-specific mappings 仍保留为 constructor 兼容输入及只读投影 | 下游调用者不再构造旧 mapping 后删除兼容入口；opcode-specific permission、directory、backing 与 residency effect 继续留在 participant transition，不并入 evidence ledger |
...
- CHI Issue H 已完成构造依赖、判定作用域与表示/运输三张视图的边界审计，并有受限 direct-Home
  ...
  Home 不自行推断。同时启用 CleanUnique 与 dirty WriteBack 的同构 requester set 已形成 resolved XP witness；等待者
  ...
  resolved witness 仍闭合 topology/flow。`WriteEvictFull(CAH=1)` modifier 另让 RN 只从 clean
  ...
  修改该行，不证明 Home 当前仍持有 copy。首个 Home profile 因此要求
  ...
  `CompDBIDResp→CopyBackWrData_UC`。这项 current-copy 检查是模型的窄化策略，不是通用 CHI 规则；
  ...
  system-derived `SNOOP_CANCELED` 只退休旧 reservation，不删除或覆盖期间形成的新 owner、reference
  backing 或 clean residency。response 前 `SnpShared` 的非失效分支也已闭合：RN 从 `UC→SC`、返回
  ...
  `CURRENT_SHARED_HOLDER` admission；该证据不是 wire field。2RN+HN+XP resolved witness 在旧 RN
  ...
  暂停 WBF REQ 的 router capture，CleanUnique 退休后再释放。该选择表示顺序，不声明周期或物理延迟。与 flow gallery
