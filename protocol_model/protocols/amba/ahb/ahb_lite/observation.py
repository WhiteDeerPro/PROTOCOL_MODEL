...
from .definition import AHB_BURSTS, AhbLiteConfig, build_ahb_lite_interface
...
class AhbObservationSession(
    ...
    def __init__(
        ...
        *,
        ...
        self.protocol = protocol or build_ahb_lite_interface(config)
        expected = {
            "READ",
            "WRITE",
            "WRITE_DATA",
    ...
    def initial_state(self) -> AhbObservationState:
        return AhbObservationState(
            None, None, None, False, self.interface_session.initial_state()
        )
    ...
    def step(
        ...
        if state.pending_address is None and not signals.hready:
            return self._fault(
                state,
                "idle_wait",
                "HREADY cannot extend an IDLE/BUSY data phase in this normalized interface",
            )
        ...
        interface_state = state.interface_state
        emissions: tuple[CanonicalEvent, ...] = ()
        if actions:
            transition = self.interface_session.step_batch(state.interface_state, actions)
            if transition.fault is not None:
                return SemanticStep(state, fault=transition.fault)
            interface_state = transition.state
