...
本文供维护者选择对外表述和复核宣称边界，不代替当前实现状态文档。
...
解释、协议变体派生，以及从单 interface connection 进入微小系统。项目价值是否成立，应由这些路径的可执行证据来判断，
不能只由架构图或代码量判断。
...
| Chisel/Diplomacy、SpinalHDL | 参数化硬件构造、协商和互连生成 | 验证构造结果的 interface/system 通信契约 | 从硬件生成配置派生模型或做边界校验 |
| CIRCT/Handshake/ESI | 硬件 IR、typed channel 和 lowering | 更靠近协议要求、运行约束与证据 | 未来交换 typed interface/transaction 描述 |
| SystemC TLM、gem5 | 系统级功能和性能模拟 | 强调具体接口行为、资源生命周期和违规定位 | 在事务级和 pin observation 之间提供协议语义层 |
...
| 宣称 | 当前证据 | 使用时的限定 |
|---|---|---|
| 从基础语义组合 InterfaceProtocol | semantic fragment、pattern 和 AMBA interface builders | 以当前实现的协议行为为范围 |
| AXI4 模型覆盖多种非平凡行为 | burst、narrow/unaligned、read interleave、AW/W/B、interface-local exclusive eligibility、observation | requirement catalog 尚未逐条闭合；exclusive 不包含跨接口冲突失效 |
...
> InterfaceProtocol，再把协议附着到 VirtualDut 并连接为 SystemProtocol；当前统一的 24 场景 AXI4 介绍集陈列五类
> 代表性行为，每案都生成波形和因果证据，其中两案进一步解释合法行为与单点违规。统一 CLI、
> replay schema 和 bridge 展示继续收敛。

M2/M3 完成后的技术预览建议用语：

> 这个 technical preview 以 AXI4 为主要样例。单接口场景集覆盖 burst、ID ordering、read interleave、
> AW/W/B correlation、narrow/unaligned 和 interface-local exclusive eligibility；独立的同宽、对齐
