...
[`migration-status.md`](../docs/architecture/migration-status.md) 为准。

## 四个入口

| 入口 | 内容 | 何时修改 |
|---|---|---|
| [传播策略](strategy/README.md) | 受众、定位、渠道、阶段、指标、演示编排和 claims/evidence 审计 | 传播判断或证据门槛变化时 |
...
策略文档不是宣传稿，宣传稿不定义实现状态，生成结果也不由人工改写。四个入口通过链接协作，不复制逐场景
清单或手写第二套统计。
...
UVM、cocotb、形式化工具的替代品。更完整的对外措辞和证据限定见
[定位文档](strategy/positioning.md)与[宣称审计](strategy/claims-and-evidence.md)。
...
- `showcase/strategy/`：人工维护的内部传播判断与审计底稿；
