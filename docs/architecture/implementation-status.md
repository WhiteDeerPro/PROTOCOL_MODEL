...
| CHI read/coherence lifecycle | 受限 direct-Home `ReadNoSnp→CompData`、clean `ReadShared`/`ReadUnique`、clean `ReadUnique→RetryAck→PCrdGrant→credited ReadUnique`、dirty-owner transfer、MESI no-SD `ReadNotSharedDirty`、显式 `UD` `WriteBackFull`、clean-peer `CleanUnique` 与 restricted shared-dirty-peer `CleanUnique` 已在 participant、packet-delivery session 和 topology-backed network session 中闭合。协议中立 `CacheLineStore/CacheCore` 持有 RN resident payload；`FullLineBackingCore/LineBackingState` 以 fixed-resident full-line、pure prepare 和 line-local version CAS 持有 Home reference payload。`ChiHomeDirectoryEntry` 只保存 sharer/owner authority，不再复制 data。coherent Retry 复用 Issue-H family-local Request-Retry/P-Credit 合同：拒绝阶段不分配 Snoop/DBID、不建立 coherence pending、不改 directory/backing；Grant 预留真实 Home transaction slot，credited reissue 原子消费 `(Requester, PCrdType)` reservation 后才进入既有 `SnpUnique→CompData→CompAck`。clean-only CleanUnique 不重取 requester 数据、不产生 prepared write、不改变 backing version；shared-dirty CleanUnique 与 dirty `ReadNotSharedDirty` 在发送 completion 前保存 `prepared_backing_write`，保持同址 reservation 到 `CompAck`，再以一个 Home-state transition 提交 backing、directory 与 pending retire；stale/double intent 保留原 state 并报告 fault。`CopyBackWrData` 在 DAT 接收 transition 中 prepare+commit。Cache 与 Home behavior 均可绑定到已有 canonical declaration，binder 不创建 port/connection；Home binder 拒绝带独立 executable backend 的 Vdut，避免两份 payload runtime authority。这里的 reference commit 不是独立 Memory VirtualDut/SN-F physical commit，也没有 HN→SN `WriteNoSnp` witness。八个 coherence profile 分别登记为构造期 feature；Home/Snoopee 仍从 selected address claim、Home authority 与 coherence domain 派生，network session 从 closed flow evidence 建立 route 并自动推进 participant egress、Link/router 与 endpoint delivery。当前缺口包括 `MakeUnique`、clean `Evict`、自动 victim/writeback scheduling、error completion 与 Retry/Snoop 并发组合、Retry cancel/multi-waiter policy、完整 same-line transient/hazard、多 pending emission batch、一般 `SD` 生成/dirty-shared/Owned lifecycle、forwarding snoop、真实 snoop filter、独立 Memory/SN protocol commit、narrow completion、多 waiter 公平性和 multi-Home/SAM runtime；有界运行耗尽仍为 inconclusive，不是 deadlock proof |
...
- CHI Issue H 已完成构造依赖、判定作用域与表示/运输三张视图的边界审计，并有受限 direct-Home
  ...
  `AddressTarget`；coherent Home 则使用支持延迟 line commit 的 `FullLineBackingCore`，不把
