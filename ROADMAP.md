...
自 v0.2 架构迁移起，`network` 是 `SystemProtocol` 内部的 topology 概念；全局可观察通信合同
本身称为 SystemProtocol。v0.1 方法已经完成审计，历史包不再保留在当前源码树；当前设计分别使用
`LinkProtocol`、`ProtocolLink/SystemProtocol` 和 topology/runtime 表达协议定义、绑定与执行。
...
[迁移状态](docs/architecture/migration-status.md)。Bridge 的概念和首版施工边界分别由
[Bridge 与类型化事务转译](docs/architecture/typed-transaction-translation.md)和
[类型化事务转译 V1 实施案](docs/architecture/translation-v1-plan.md)维护。
...
### v0.2：Typed Bridge and Composable Network
