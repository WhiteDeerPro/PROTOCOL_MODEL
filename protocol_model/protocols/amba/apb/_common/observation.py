...
class ApbPhaseObservationSession(
    ...
    The engine observes one point-to-point APB interface. Variant packages provide
    ...
    def __init__(
        ...
        *,
        ...
        if set(protocol.event_kinds) != expected:
        ...
        self.interface_session = protocol.open_session()
    ...
    def _state(
        ...
        return self.state_type(
            pending_request, interface_state, last_tick, wakeup_hold
        )

    def initial_state(self) -> ApbPhaseObservationState:
        return self._state(None, self.interface_session.initial_state(), None)
    ...
    def step(
        ...
        if reset:
            return SemanticStep(
                self._state(
                    None, self.interface_session.initial_state(), frame.tick
                )
            )
        ...
        if state.pending_request is None:
            ...
            if not getattr(signals, "psel"):
                return SemanticStep(
                    self._state(
                        None, state.interface_state, frame.tick, wakeup_hold
                    )
                )
            request = self._request(signals, frame)
            transition = self.interface_session.step(state.interface_state, request)
        ...
        transition = self.interface_session.step(state.interface_state, response)
        if transition.fault is not None:
            return SemanticStep(state, fault=transition.fault)
        normalized = transition.emissions[0]
        return SemanticStep(
