"""State-driven generation for the AXI4 read-channel InterfaceProtocol implementation."""
...
from protocol_model.interface import InterfaceProtocol, InterfaceTrace
from protocol_model.semantics import EventOffer

from .definition import Axi4Config, build_axi4_interface, build_axi4_read_interface
...
class Axi4ReadGenerator:
    def __init__(self, protocol: InterfaceProtocol | None = None) -> None:
        self.protocol = protocol or build_axi4_read_interface()
        if not {"AR", "R"}.issubset(self.protocol.event_kinds) or "axi4.read" not in self.protocol.monitors:
    ...
    def from_config(cls, config: Axi4Config | None = None) -> "Axi4ReadGenerator":
        return cls(build_axi4_read_interface(config))
    ...
    def generate(
        ...
        return InterfaceTrace(tuple(events), state.causal_edges)
...
class Axi4WriteGenerator:
    def __init__(self, protocol: InterfaceProtocol | None = None) -> None:
        self.protocol = protocol or build_axi4_interface()
        if set(self.protocol.event_kinds) != {"AW", "W", "B", "AR", "R"}:
            raise ValueError("AXI4 write generator requires the five-channel interface")

    @classmethod
    def from_config(cls, config: Axi4Config | None = None) -> "Axi4WriteGenerator":
        return cls(build_axi4_interface(config))
