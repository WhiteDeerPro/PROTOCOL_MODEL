...
class AhbLiteInterfaceTest(unittest.TestCase):
...
class Ahb5InterfaceTest(unittest.TestCase):
    @staticmethod
    ...
    def test_issue_c_properties_shape_the_payload(self) -> None:
        ...
        self.assertEqual(7, protocol.event_kinds["READ"].schema.fields["prot"].domain.width)
        self.assertIn("nonsecure", protocol.event_kinds["READ"].schema.fields)
        self.assertIn("auser", protocol.event_kinds["WRITE"].schema.fields)
        self.assertIn("strb", protocol.event_kinds["WRITE_DATA"].schema.fields)
        self.assertIn("wuser", protocol.event_kinds["WRITE_DATA"].schema.fields)
        self.assertIn("ruser", protocol.event_kinds["READ_RESPONSE"].schema.fields)
        self.assertIn("buser", protocol.event_kinds["WRITE_RESPONSE"].schema.fields)

    def test_write_strobes_are_sparse_write_attributes_not_lane_legality(self) -> None:
        protocol = build_ahb5_interface(Ahb5Config(write_strobes=True))
        events = (
            CanonicalEvent(
                "WRITE",
                None,
    ...
    def test_exclusive_shape_and_response_signaling_are_interface_checked(self) -> None:
