...
class AhbBurstMonitor(
    ...
    def step(
        ...
        if event.trace_index is None:
            return self._fault(
                state, "trace_index", "AHB requests must be normalized by an InterfaceSession"
            )
...
class AhbWriteDataMonitor(
    ...
    def step(
        ...
        if event.trace_index is None:
            return self._fault(
                state, "trace_index", "AHB write events require InterfaceSession indices"
            )
        if event.kind == "WRITE":
...
def build_ahb_lite_interface(config: AhbLiteConfig | None = None) -> InterfaceProtocol:
    config = config or AhbLiteConfig()
    event_kinds = {
        "READ": InterfaceEventKind(
            "READ", "manager", "subordinate", _read_schema(config)
        ),
        "WRITE": InterfaceEventKind(
            "WRITE", "manager", "subordinate", _write_schema(config)
        ),
        "WRITE_DATA": InterfaceEventKind(
            "WRITE_DATA", "manager", "subordinate", _write_data_schema(config)
        ),
        "READ_RESPONSE": InterfaceEventKind(
            "READ_RESPONSE",
            "subordinate",
            "manager",
            _read_response_schema(config),
        ),
        "WRITE_RESPONSE": InterfaceEventKind(
            "WRITE_RESPONSE",
            "subordinate",
            "manager",
