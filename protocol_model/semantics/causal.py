"""Finite strict partial orders for protocol execution traces."""
...
class CausalGraph:
    ...
    def topological_order(self) -> tuple[Node, ...]:
        ...
        serial = {node: index for index, node in enumerate(self._successors)}
        ready: list[tuple[str, int, Node]] = []
        for node, degree in indegree.items():
            if degree == 0:
                heappush(ready, (repr(node), serial[node], node))
        ordered: list[Node] = []
        while ready:
            ...
            for successor in self._successors[node]:
                ...
                if indegree[successor] == 0:
                    heappush(
                        ready,
                        (repr(successor), serial[successor], successor),
                    )
