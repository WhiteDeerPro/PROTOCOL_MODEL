...
from protocol_model.semantics import EventField, EventSchema
...
from ..ahb_lite import AhbLiteConfig, build_ahb_lite_interface
...
class Ahb5Config:
    ...
    def __post_init__(self) -> None:
        ...
        if self.master_width > 8:
            raise ValueError("this AHB5 interface profile supports manager IDs up to 8 bits")
...
class Ahb5ExclusiveSignalMonitor(
    ...
    """Check interface-visible HEXCL/HEXOKAY response legality.
    ...
    def step(
        ...
        if event.trace_index is None:
            return self._fault(
                state, "trace_index", "AHB5 events require InterfaceSession indices"
            )
...
def build_ahb5_interface(config: Ahb5Config | None = None) -> InterfaceProtocol:
    """Build an AHB5 interface with selected Issue C properties."""

    config = config or Ahb5Config()
    base = build_ahb_lite_interface(
        AhbLiteConfig(config.address_width, config.data_width)
    )
    event_kinds = {}
    for name, event_kind in base.event_kinds.items():
        if name in {"READ", "WRITE"}:
            event = _request_schema(event_kind.schema, config)
        elif name == "WRITE_DATA":
            event = _write_data_schema(event_kind.schema, config)
        elif name == "READ_RESPONSE":
            event = _response_schema(event_kind.schema, config, read=True)
        else:
            event = _response_schema(event_kind.schema, config, read=False)
        event_kinds[name] = InterfaceEventKind(
            name, event_kind.source_role, event_kind.destination_role, event
        )
