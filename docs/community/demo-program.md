...
[`showcase/demos/axi4`](../../showcase/demos/axi4/README.md) 为 canonical 入口；本页不复制 case 清单，以免实现与宣传计划分别
维护两套事实。
...
AXI4→APB bridge 适合作为统一 AXI4 介绍之后的第二故事，用来展示两种 InterfaceProtocol、bridge VirtualDut 和
SystemProtocol 的协作：
...
最小 2×2 AXI4-Lite crossbar 已作为独立 VirtualDut 故事发布，用来显示 ingress FIFO、地址归属、仲裁和
return ownership。它目前是 deterministic execution witness；系统级 fairness、wait-for/deadlock 与外部 RTL
conformance 仍作为后续研究故事，不由这一张争用轨迹代替。
...
实现细节、运行命令和场景导航属于
[`showcase/demos/axi4`](../../showcase/demos/axi4/README.md)；发布用文案和录屏脚本属于
[`showcase/materials`](../../showcase/materials/README.md)。本页只在编排原则或发布门槛变化时更新。
