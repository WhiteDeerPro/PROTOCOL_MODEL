...
def build_ace_lite_data_interface(
    ...
    axi4 = build_axi4_interface(config.axi4_config())
    channels = {}
    for name, channel in axi4.event_kinds.items():
        event = channel.schema
        if name in ("AR", "AW"):
            event = _coherent_address_schema(event, read=name == "AR")
        manager_to_interconnect = name in ("AR", "AW", "W")
        channels[name] = InterfaceEventKind(
            name,
    ...
    return InterfaceProtocol.define(
        "ace_lite_data",
        interface_family=ACE_LITE_FAMILY,
