...
| typed representation / codec | operation translation 已有；CHI Issue H 已把 typed protocol message、named logical-field record、`ChiNetworkPacket` 与 `ChiProtocolFlit` 分成显式对象。当前 message profile 覆盖 `ReadNoSnp`/`ReadShared`/`ReadUnique`/`ReadNotSharedDirty`/`CleanUnique`/`WriteBackFull`/`PCrdReturn` REQ、`SnpResp`/`Comp`/`CompAck`/`CompDBIDResp`/`RetryAck`/`PCrdGrant` RSP、`CompData`/`SnpRespData`/`CopyBackWrData` DAT 与 `SnpShared`/`SnpUnique`/`SnpNotSharedDirty`/`SnpCleanInvalid` SNP；`ChiIssueHLogicalFieldCodec` 为当前 20 个 protocol-message form 提供 channel+opcode 分派、当前受限 typed profile 的精确字段集合、常量/宽度/profile 诊断及双向 round-trip，其中包括 CleanUnique 与既有 writeback forms。各 channel 的 `LCrdReturn` 保持为不进入 message codec 和 packet 的 hop-local link flit。`SnpUnique`、`SnpCleanInvalid` 与 no-SD `SnpNotSharedDirty` profile 检查 `DoNotGoToSD=1`；`SnpCleanInvalid` 另固定 `RetToSrc=0`，但受限 dirty peer 仍以 `SnpRespData_I_PD` 交回 dirty data/responsibility，clean peer 才使用无数据 `SnpResp_I`。Network packet 单独持有 source/target route identity 与 packet index/count；当前尚不拆分 message 内容，Snoop fanout 则产生若干各自为单 fragment 的目标副本。Issue H 每 packet 恰好一 protocol flit、每 flit 一 phit，但 phit/raw pin、packed bit layout、完整 opcode/conditional-field catalog 和实际 multi-packet DAT splitter/reassembler 尚未实现 |
...
| CHI read/coherence lifecycle | 受限 direct-Home `ReadNoSnp→CompData`、clean `ReadShared`/`ReadUnique`、dirty-owner transfer、MESI no-SD `ReadNotSharedDirty`、显式 `UD` `WriteBackFull`、clean-peer `CleanUnique` 与 restricted shared-dirty-peer `CleanUnique` 已在 participant、packet-delivery session 和 topology-backed network session 中闭合。clean-only 路径执行 `SC requester → CleanUnique → SnpCleanInvalid → clean SnpResp_I → Comp_UC → CompAck → UC requester`，不重取 requester 数据、不改变 backing，也不声明 DAT。`CHI_FEATURE_CLEAN_UNIQUE_SHARED_DIRTY_PEER` 依赖该 feature，仅增加 Snoopee→Home DAT 及相应 capability；预置的唯一 `SD` peer 执行 `SnpCleanInvalid → SnpRespData_I_PD → I`，Home 以 `ChiCoherentTransactionPending.dirty_result` 持有最新数据和 reference memory-update obligation，保持同址 reservation 到 `CompAck`，再原子更新融合式 reference backing、清除 `shared_dirty_owner` 并提交 requester 为 unique owner。该 “memory update” 不是独立 Memory VirtualDut/SN-F physical commit；当前也没有 coherent Home/main-memory CHI attach/bind recipe。协议中立 `CacheLineStore/CacheCore` 持有 resident payload，RN participant 的基线 permission 是 `I/UC/SC/UD`，`SD` 只作为上述 feature 可预置、只由 `SnpCleanInvalid` 消费的受限状态；Home 持有 directory/reference backing、`unique_owner/shared_dirty_owner`、Snoop TxnID 与 DBID pending。静止点 monitor 检查 directory、permission 和数据权威。Cache behavior 可绑定到已有 canonical VirtualDut，binder 不创建 port 或 topology connection。七个 coherence profile 分别登记为构造期 feature；shared-dirty extension 不把 DAT 反向要求到 clean-only construction。Home/Snoopee 仍从 selected address claim、Home authority 与 coherence domain 派生，network session 从 closed flow evidence 建立 route 并自动推进 participant egress、Link/router 与 endpoint delivery；场景只提交 operation 并显式调用离散 advance/run。当前缺口包括 `MakeUnique`、clean `Evict`、自动 victim/writeback scheduling、Retry/error 与 coherence 的组合、完整 same-line transient/hazard、多 pending emission batch、一般 `SD` 生成/dirty-shared/Owned lifecycle、forwarding snoop、真实 snoop filter、独立 Memory/SN commit、narrow/error completion、多 waiter 公平性和 multi-Home/SAM runtime；有界运行耗尽仍为 inconclusive，不是 deadlock proof |
...
| CHI participant / VirtualDut composition | cache `attach` 从 `CacheCore` 创建第一个 VirtualDut；cache binder 接收既有 canonical VirtualDut、CacheCore 与 port-channel map，并让 assembly/facet 保持同一 object identity。binder 只形成 participant/facet，不连接网络；resolver 核对 object identity、NodeID、channel flow 与 authority，不创建、复制或修改 VirtualDut。`ChiAddressHomeNode(AddressTarget)` 只覆盖 direct ReadNoSnp adapter；coherent Home/main-memory 尚无对等 attachment、recipe 或 binder，reference backing 仍融合在 Home state | 先把 coherent Home 拆为协议中立 backing target 与 directory/transaction facet，再为已有 canonical Home/Memory VirtualDut 建 binder；若验证目标需要独立 SN-F，再显式构造 HN→SN flow 和 physical commit witness。第二种 participant 复用后才提取通用 binder API |
...
- CHI Issue H 已完成构造依赖、判定作用域与表示/运输三张视图的边界审计，并有受限 direct-Home
  ...
  `CompAck` 后更新融合式 reference backing 与 unique directory；它不包含独立 Memory/SN physical commit，
  也不声称完整 MOESI/Owned。这些 lifecycle 分别登记为构造期 feature；dirty-unique 依赖 clean ReadUnique，而独立
  ...
  派生。`MakeUnique`、clean `Evict`、自动 victim/writeback scheduling、一般 `SD` 生成与 MOESI
  Owned lifecycle、真实 snoop filter、Home/Memory attachment 及更一般的 transient/hazard 仍未进入该
  slice。两级 XP 的公开
  ...
  核心功能。aligned full-DAT-width direct read 已通过 `ChiAddressHomeNode` 委派给协议无关
  `AddressTarget`，但 coherent Home reference backing 尚未迁到该 seam；功能缺口
